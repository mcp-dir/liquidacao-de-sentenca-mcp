...
### Liquide débitos judiciais com correção, juros e honorários, para IA

**Liquidação de sentença** e de débitos para IA. Some parcelas e abatimentos, aplique correção oficial (IPCA-E, INPC, SELIC), juros, multa e honorários, e chegue no valor atualizado. Índices ao vivo, sem login.

- ⚖️ **Liquida débitos**: parcelas, abatimentos, correção, juros e honorários
...
**Manual** (se o deeplink não abrir): [claude.ai/customize/connectors](https://claude.ai/customize/connectors?surface=cowork) → **+** → **Adicionar conector personalizado** → cole **Nome** `Liquidação de Sentença` e **URL** `https://api.mcp.ai/p_liquidacao-de-sentenca`.

### Cursor

[➕ Instalar Liquidação de Sentença no Cursor](cursor://anysphere.cursor-deeplink/mcp/install?name=liquidacaodesentenca&config=eyJ1cmwiOiJodHRwczovL2FwaS5tY3AuYWkvcF9saXF1aWRhY2FvLWRlLXNlbnRlbmNhIn0=)

### VS Code (Copilot Chat)

[➕ Instalar Liquidação de Sentença no VS Code](vscode:mcp/install?name=liquidacaodesentenca&config=%7B%22type%22%3A%22http%22%2C%22url%22%3A%22https%3A%2F%2Fapi.mcp.ai%2Fp_liquidacao-de-sentenca%22%7D)
...
Atualize a condenação de R$ 25.000 pela SELIC desde a citação
```
...
MIT — veja [LICENSE](LICENSE). O servidor MCP em `api.mcp.ai/p_liquidacao-de-sentenca` é proprietário (hosted); este repositório (manifestos, docs, skills) é MIT.
