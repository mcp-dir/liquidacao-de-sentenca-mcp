---
name: liquidacao-de-sentenca-mcp
description: Liquidação de sentença e de débitos: some parcelas e abatimentos, aplique correção oficial, juros, multa e honorários. Use quando o usuário pedir para liquidar, executar ou apurar o valor atualizado de uma condenação ou dívida.
---

# Liquidação de Sentença — REST API skill

Você tem acesso à **Liquidação de Sentença** REST API na MCP.AI.
...
Este MCP também funciona via **conexão MCP** (Claude / Cursor) em `https://api.mcp.ai/p_liquidacao-de-sentenca` — veja o [README](../../README.md). A skill acima é pra consumir a **REST API** direto (agente próprio / código).
