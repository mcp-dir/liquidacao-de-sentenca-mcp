...
### Liquidate court debts with correction, interest and fees, for AI

**Judgment and debt liquidation** for AI. Sum installments and payments, apply official correction (IPCA-E, INPC, SELIC), interest, penalty and fees, and reach the updated amount. Live indices, no login.

- ⚖️ **Liquidates debts**: installments, payments, correction, interest and fees
...
**Manual** (if the deeplink does not open): [claude.ai/customize/connectors](https://claude.ai/customize/connectors?surface=cowork) → **+** → **Add custom connector** → paste **Name** `Liquidação de Sentença` and **URL** `https://api.mcp.ai/p_liquidacao-de-sentenca`.

### Cursor

[➕ Install Liquidação de Sentença in Cursor](cursor://anysphere.cursor-deeplink/mcp/install?name=liquidacaodesentenca&config=eyJ1cmwiOiJodHRwczovL2FwaS5tY3AuYWkvcF9saXF1aWRhY2FvLWRlLXNlbnRlbmNhIn0=)

### VS Code (Copilot Chat)

[➕ Install Liquidação de Sentença in VS Code](vscode:mcp/install?name=liquidacaodesentenca&config=%7B%22type%22%3A%22http%22%2C%22url%22%3A%22https%3A%2F%2Fapi.mcp.ai%2Fp_liquidacao-de-sentenca%22%7D)
...
```
Liquidate this judgment: debt installments + payments, IPCA-E + interest + 10% fees
Update the R$25,000 award by SELIC since the summons
```
...
- 📧 [liquidacaodesentenca@mcp.ai](mailto:liquidacaodesentenca@mcp.ai)
- 🐛 [GitHub Issues](https://github.com/mcp-dir/liquidacao-de-sentenca-mcp/issues)
- 📄 [docs/](docs/)
...
MIT — see [LICENSE](LICENSE). The MCP server at `api.mcp.ai/p_liquidacao-de-sentenca` is proprietary (hosted); this repo (manifests, docs, skills) is MIT.
